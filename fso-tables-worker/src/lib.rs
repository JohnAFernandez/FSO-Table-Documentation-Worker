...
use is_number::{is_integer, is_float};
use email_address::{EmailAddress};

mod db_fso; use db_fso::*;
mod email_fns; use email_fns::*;
mod secrets;
...
async fn fetch(req: Request, env: Env, _ctx: Context,) -> worker::Result<Response> {
    ...
    Router::new()
        ...
        .put_async("/api/users/upgrade", user_upgrade_user_permissions)
        ...
        .put_async("/api/users/downgrade", user_downgrade_user_permissions)
            ...
            match req.json::<NewItem>().await{
                Ok(new_item) => {
                    ...
                    if new_item.table_id < -1 {
                        return err_specific("{\"Error\":\"Table must be specified.\"}".to_string()).await
                    }
